...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      ...
      {/* TOP BAR */}
      <div style={{
        background:'var(--dark)', padding:'0 1.5rem', height:56,
        display:'flex', alignItems:'center', justifyContent:'space-between',
        position:'sticky', top:0, zIndex:100, flexShrink:0,
      }}>
        <div style={{ display:'flex', alignItems:'center', gap:'.75rem' }}>
          <button onClick={() => setCollapsed(c => !c)} style={{ background:'none', border:'none', color:'rgba(255,255,255,.5)', fontSize:'1.2rem', cursor:'pointer', padding:'.2rem' }}>☰</button>
          <span style={{ color:'#fff', fontFamily:"'Playfair Display',serif", fontSize:'1.1rem', display:'flex', alignItems:'center', gap:'.4rem' }}><span className="admin-logo-icon"><BrandMark size="100%" /></span>{companyName}</span>
          <span style={{ fontSize:'.7rem', background:'rgba(99,102,241,.35)', color:'#a5b4fc', padding:'2px 9px', borderRadius:50, fontFamily:"'Inter',sans-serif", fontWeight:700 }}>ADMIN</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
          ...
          <button onClick={handleLogout} style={{ background:'#ef4444', color:'#fff', border:'none', padding:'.35rem .9rem', borderRadius:8, fontSize:'.82rem', fontWeight:600, cursor:'pointer' }}>
            🚪 Logout
          </button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden' }}>
        ...
        <div style={{
          ...
          padding:'1rem 0', flexShrink:0, transition:'width .25s ease',
          overflow:'hidden',
        }}>
          {!collapsed && (
          ...
          {SIDEBAR.map(item => (
            ...
            >
              ...
              {!collapsed && item.key==='requests' && pending.length>0 && (
                <span className="badge-count">{pending.length}</span>
              )}
              {!collapsed && item.key==='feedback' && unreadFeedback.length>0 && (
                <span className="badge-count">{unreadFeedback.length}</span>
              )}
            </button>
          ))}
          <div style={{ marginTop:'auto', padding:'0 .5rem' }}>
            <button className="sidebar-link" onClick={handleLogout} style={{ justifyContent: collapsed ? 'center' : 'flex-start' }}>
          ...
          <AnimatePresence mode="wait">
            ...
            {tab==='settings' && settingsForm && (
              <motion.div key="settings" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
                ...
                {/* SEAT LIMITS */}
                <div style={{ background:'var(--white)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'1.5rem', marginBottom:'1.5rem' }}>
                  <div style={{ fontWeight:700, marginBottom:'1.25rem' }}>💺 Seat Limits</div>
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'1rem' }}>
                    <div className="form-group">
                      <label className="form-label">❄️ AC Seats From</label>
                      <input type="number" min={1} value={settingsForm.acSeatFrom}
                        onChange={e=>setSettingsForm({...settingsForm, acSeatFrom: e.target.value})} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">❄️ AC Seats To</label>
                      <input type="number" min={1} value={settingsForm.acSeatTo}
                        onChange={e=>setSettingsForm({...settingsForm, acSeatTo: e.target.value})} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">🌿 Non-AC Seats From</label>
                      <input type="number" min={1} value={settingsForm.nonAcSeatFrom}
                        onChange={e=>setSettingsForm({...settingsForm, nonAcSeatFrom: e.target.value})} />
                    </div>
                    <div className="form-group">
                      <label className="form-label">🌿 Non-AC Seats To</label>
                      <input type="number" min={1} value={settingsForm.nonAcSeatTo}
                        onChange={e=>setSettingsForm({...settingsForm, nonAcSeatTo: e.target.value})} />
                    </div>
                  </div>
                  <p style={{ fontSize:'.76rem', color:'var(--text-3)', marginTop:'.75rem' }}>
                    ❄️ AC will show seats numbered {settingsForm.acSeatFrom} to {settingsForm.acSeatTo}
                    ({Math.max(0, (Number(settingsForm.acSeatTo)||0) - (Number(settingsForm.acSeatFrom)||0) + 1)} seats) ·
                    🌿 Non-AC will show seats numbered {settingsForm.nonAcSeatFrom} to {settingsForm.nonAcSeatTo}
                    ({Math.max(0, (Number(settingsForm.nonAcSeatTo)||0) - (Number(settingsForm.nonAcSeatFrom)||0) + 1)} seats) — no upper limit, set any range (e.g. 1–400, or 400–500 for an extension room).
                  </p>
                </div>

                <button className="btn btn-primary" disabled={savingSettings} onClick={saveSettings}>
                  {savingSettings ? 'Saving…' : '💾 Save Seat Settings'}
                </button>
                </>)}
                ...
                <div style={{ background:'var(--white)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'1.5rem', marginBottom:'1.5rem' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:'1.5rem' }}>
                    {[
                      { key:'acQrImage',    label:'❄️ AC Zone QR' },
                      { key:'nonAcQrImage', label:'🌿 Non-AC Zone QR' },
                    ].map(({key,label}) => (
