...
export default function BookPage() {
  ...
  /* ══════════════════════════════════════════════════════
     NON-AC ROOM — simple seat grid, admin-set range
     (from settings.nonAcSeatFrom to settings.nonAcSeatTo — no upper limit)
  ══════════════════════════════════════════════════════ */
  const nonAcFrom = settings.nonAcSeatFrom ?? 1;
  const nonAcTo   = settings.nonAcSeatTo   ?? 90;
  const NONAC_NUMS = [];
  for (let n = nonAcFrom; n <= nonAcTo; n++) NONAC_NUMS.push(n);
  const NONAC_ROWS = [];
    ...
    <div className="ac-wrap">
      ...
      <div className="ac-legend">
        ...
        <span className="ac-leg-item" style={{ marginLeft: 'auto', fontWeight: 600 }}>
          {seats.filter(s => s.type === 'NON_AC' && s.status === 'available').length} / {NONAC_NUMS.length} available
        </span>
      </div>
    </div>
  );
  ...
  /* ══════════════════════════════════════════════════════
     AC ROOM — simple seat grid, admin-set range
     (from settings.acSeatFrom to settings.acSeatTo — no upper limit)
  ══════════════════════════════════════════════════════ */
  const acFrom = settings.acSeatFrom ?? 1;
  const acTo   = settings.acSeatTo   ?? 60;
  const AC_NUMS = [];
  for (let n = acFrom; n <= acTo; n++) AC_NUMS.push(n);
  const AC_ROWS = [];
  ...
  const AcRoom = () => (
    <div className="ac-wrap">
      ...
      <div className="ac-legend">
        ...
        <span className="ac-leg-item" style={{ marginLeft: 'auto', fontWeight: 600 }}>
          {seats.filter(s => s.type === 'AC' && s.status === 'available').length} / {AC_NUMS.length} available
        </span>
      </div>
    </div>
  );
  ...
  return (
    <main className="page" style={{ background:'var(--lighter)' }}>
      <Helmet>
        ...
        <meta name="keywords" content={`book reading room seat online, library seat booking ${settings.city}, monthly reading room fees, AC seat booking, non AC seat booking, reading room availability`} />
