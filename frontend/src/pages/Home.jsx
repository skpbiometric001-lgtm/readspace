...
const FEATURES = [
  { icon: '❄️', bg: '#eef2ff', title: 'AC Comfort Zone', desc: 'Premium air-conditioned halls with ergonomic chairs, ample lighting and zero disturbance policy.' },
  { icon: '🌿', bg: '#f0fdf4', title: 'Non-AC Budget Zone', desc: 'Well-ventilated, naturally lit spaces perfect for focused study at an affordable price.' },
...
export default function Home() {
  ...
  const [settings, setSettings] = useState({ acPrice: 1499, nonAcPrice: 900 });
  ...
  const startingPrice = Math.min(settings.acPrice ?? 1499, settings.nonAcPrice ?? 900);

  return (
    <main className="page">
      <Helmet>
        <title>{`${companyName} ${city} — Best AC & Non-AC Reading Room / Library | Book Seat Online`}</title>
        <meta name="description" content={`Book your AC or Non-AC study seat online at ${companyName}, ${city}'s most modern reading room and library. High-speed Wi-Fi, curated library, 24/7 CCTV security & real-time seat booking. Ideal for UPSC, MPSC & competitive exam preparation.`} />
        <meta name="keywords" content={`reading room in ${city}, library in ${city}, AC reading room ${city}, non AC reading room ${city}, study room ${city}, best library for UPSC preparation, MPSC study room, monthly seat booking library`} />
        ...
        <div className="hero-content">
          <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: .6 }}>
            ...
            <p className="hero-desc">
              India's most modern reading rooms with dedicated AC & Non-AC zones, high-speed Wi-Fi,
              curated library and real-time seat booking — right at your fingertips.
            </p>
            ...
            <div className="hero-stats">
              {[['120+', 'Total Seats'], ['AC & Non-AC', 'Two Zones'], [`₹${startingPrice}/mo`, 'Starting at'], ['24/7', 'CCTV Security']].map(([val, label]) => (
          ...
          <motion.div className="hero-visual" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .8, delay: .2 }}>
            <div className="hero-card-stack">
              ...
              <div className="hero-badge b1">❄️ AC Zone Available</div>
      ...
      <section className="section" style={{ background: '#fff' }}>
        <div className="section-inner">
          <div className="section-header">
            <span className="section-eyebrow">Choose Your Zone</span>
            <h2>Two Zones, One Goal — Your Success</h2>
            <p>Pick the environment that suits your study style and budget. Both zones include Wi-Fi, lockers, and library access.</p>
          </div>
